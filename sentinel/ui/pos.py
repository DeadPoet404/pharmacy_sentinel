...
class BrutalistPOS(QWidget):
    def __init__(self, db_manager, user_id, user_name, session_id):
        ...
        self.status_lbl = QLabel(f"➔ SENTINEL_CONSOLE // OP: {user_name.upper()} // SESS: {session_id}")
        top_layout.addWidget(self.status_lbl); layout.addWidget(top)
        ...
        self.search_table.setHorizontalHeaderLabels(["PRODUCT", "PRICE", "STOCK"])
        ...
        l_layout.addWidget(self.search_table)
        ...
        self.viz_frame = QFrame(); self.viz_frame.setFixedSize(500, 500)
        self.viz_frame.setStyleSheet("border: 2px dashed black; background: #dcdcdc;")
        vf_layout = QVBoxLayout(self.viz_frame)
        self.img_label = QLabel(); self.img_label.setAlignment(Qt.AlignCenter); vf_layout.addWidget(self.img_label)
        ...
        self.uom_tag.setStyleSheet("background: black; color: #FF4500; font-size: 40px; font-weight: 900; padding: 10px 30px;")
        ...
        self.total_lbl.setStyleSheet("font-size: 60px; font-weight: 900; border: 2px solid black; background: white; padding: 10px;")
        ...
        f_label = QLabel("F2:SEARCH | F3:STOCK_IN | F4:UOM | F8:CHECKOUT | F12:REGISTRY")
    ...
    def select_item(self, item):
        ...
        self.update_ui(it.text(), form)

    def update_ui(self, name, form):
        img_path = os.path.join(self.assets_dir, f"{form}.webp")
        if os.path.exists(img_path):
            self.img_label.setPixmap(QPixmap(img_path).scaled(450, 450, Qt.KeepAspectRatio))
            self.viz_frame.setStyleSheet("border: 2px solid black; background: white;")
    ...
    def open_checkout(self):
        ...
        if total <= 0: return
        self.checkout_ui = SettlementUI(total, self.finalize_sale)
    ...
    def finalize_sale(self, method, tendered):
        ...
        if success:
            ...
            self.cart_items = []; self.update_ui("SYSTEM_READY", "idle")
            self.total_lbl.setText("0.00"); self.search_box.clear()
        else:
            QMessageBox.critical(self, "ERROR", "TRANSACTION_FAILED. Check console for details.")
